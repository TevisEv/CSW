...
def sql_to_laravel_type(sql_type):
    ...
    if match:
        ...
        if laravel_type == 'decimal' and decimal_place:
            return f"{laravel_type}({size}, {decimal_place})"
        elif size:
            return f"{laravel_type}({size})"
        else:
            return f"{laravel_type}()"
    else:
        return 'string'
...
def generate_migration_from_sql(sql_schema, save_path=None):
    ...
    for field in fields:
        ...
        laravel_type = sql_to_laravel_type(field_type)
...
"""
    return migration_content, field_names
...
def generate_migration_only():
    ...
    if save_path:
        ...
        migration_content, field_names = generate_migration_from_sql(
            sql_schema)  # Recibe correctamente el tuple
        ...
        with open(file_path, 'w') as f:
            # Aquí es donde necesitabas asegurarte de pasar un string
